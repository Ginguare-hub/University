...
Var
    ...
    //TODO Настроить и запретить вставку

Implementation

{$R *.dfm}

Function CalculateFunction(SomeArray: TArrayOI): Integer;
Const
    EPSILON: Double = 0.000001;
Var
    MinAbsDiff, CurrAbsDiff, ArithmMean: Double;
    ArrLength, ArrHigh, I, SumOfElements, AnswerNumber: Integer;
Begin
    MinAbsDiff := 0.0;
    CurrAbsDiff := 0.0;
    ArithmMean := 0.0;
    ...
    ArrLength := Length(SomeArray);
    ArrHigh := High(SomeArray);

    For I := 0 To ArrHigh Do
    Begin
        SumOfElements := SumOfElements + SomeArray[I];
    End;

    ArithmMean := SumOfElements / ArrLength;
    MinAbsDiff := Abs(ArithmMean - SomeArray[Low(SomeArray)]);

    For I := 0 To ArrHigh Do
    Begin
        CurrAbsDiff := Abs(ArithmMean - SomeArray[I]);

        If MinAbsDiff > CurrAbsDiff Then
    ...
    Begin
        ...
        If (MinAbsDiff >= CurrAbsDiff - EPSILON) And (MinAbsDiff <= CurrAbsDiff + EPSILON) Then
            AnswerNumber := I + 1;
    End;

    CalculateFunction := AnswerNumber;
End;
...
Begin
    ...
    Begin
        ...
        MainForm.LabelResult.Visible := True;
        AnswerNumber := CalculateFunction(GridAArray);
