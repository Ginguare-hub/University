...
Type
    ...
    Function FillGridByArrayData(FromArray: TArrayOS; ToGrid: TStringGrid): ERROR_CODES;
    Function FindAmicableNumbers: TArrayOS;
...
Var
    I, SumOfDivisors, NumberDivTwo: Integer;

Begin
    I := 0;
    SumOfDivisors := 0;
    NumberDivTwo := Number Div 2;

    For I := 1 To NumberDivTwo Do
        If Number Mod I = 0 Then
            SumOfDivisors := SumOfDivisors + I;

    SumOfProperDivisors := SumOfDivisors;
End;
...
Var
    I, J, ArrLength, SumOfDivisors: Integer;
    SArray: TArrayOS;
Begin
    I := 0;
    J := 1;
    ...
    For I := 1 To MY_LIMIT Do
    Begin
        SumOfDivisors := SumOfProperDivisors(I);
        If ((I < SumOfDivisors) And (I = SumOfProperDivisors(SumOfDivisors))) Then
        Begin
            ArrLength := ArrLength + 1;
            SetLength(SArray, ArrLength);
            SArray[High(SArray)] := IntToStr(I) + ' и ' + IntToStr(SumOfProperDivisors(I)) ;
            Inc(J);
        End;
    End;
...
Begin
    ...
    While I < MainForm.StringGridA.ColCount Do
    Begin
        SetLength(StringArray, I + 1);
...
Procedure SetStringGridACorrect();
Var
    AnswerArray: TArrayOS;
Begin
    AnswerArray := Nil;
    If IsFirstNumberCorrect Then
    Begin
        AnswerArray := FindAmicableNumbers;
...
Begin
    ...
    Begin
        MainForm.LabelResult.Caption := 'Ошибка в вводе.';
        MainForm.StringGridA.Visible := False;
    End
    Else
    Begin
        MainForm.LabelResult.Caption := 'Все дружественные числа, меньшие за 100000:';
...
Begin
    ...
    InstructionForm.GuideLabel.Caption :=
        '1) Желательно не ломать приложение.'#13#10;
...
Var
    ...
    ArrayOfAnswers: TArrayOS;
Begin
    Error := NO_ERRORS;
    ArrLength := 0;
    ArrayOfAnswers := Nil;

    ArrayOfAnswers := GetStringArrayFromGrid(MainForm.StringGridA);
    ArrHigh := High(ArrayOfAnswers);

    Try
        Rewrite(SavedFile);
        WriteLn(SavedFile, 'Дружественные числа, меньшие за ' + MainForm.NumberOneEdit.Text + ':');

        For I := 0 To ArrHigh Do
        Begin
            WriteLn(SavedFile, ArrayOfAnswers[I], ' ');
        End;
...
//    End;
    ...
    Begin
        ...
        If AnswerArray = Nil Then
            MainForm.LabelResult.Caption := 'Ошибка.'
        Else
        Begin
            MainForm.LabelResult.Caption := '...';
            MainForm.StringGridA.Visible := True;
            FillGridByArrayData(AnswerArray, MainForm.StringGridA);
        End;
    End;
