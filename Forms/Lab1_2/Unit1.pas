...
//    Procedure SaveTabClick(Sender: TObject);
//    Procedure FormCloseQuery(Sender: TObject; var CanClose: Boolean);
    Function CheckStringOnValidity(TestString: String): Boolean;
...
Begin
    ...
    Begin
        Try
            ...
            If (TestInteger >= 1) And (TestInteger <= 30) Then
...
Var
    I, N, Temp, SumOfNumbers: Integer;
Begin
    AnswerLabel.Caption := '';
    SumOfNumbers := 0;
    Temp := 1;

    N := StrToInt(MainForm.NumberOneEdit.Text);
    ...
    AnswerLabel.Caption := 'Сумма по формуле равна числу: ' + IntToStr(SumOfNumbers);
End;
...
Begin
    ...
    InstructionForm.GuideLabel.Caption :=
        '1) Программа принимает исключительно положительные целые числа в диапазоне [1;30].'#13#10 +
...
Begin
    IsFirstNumberCorrect := CheckStringOnValidity(NumberOneEdit.Text);
...
Var
    Error: ERROR_CODES;
    NumberOne, SumOfNumbers: Integer;
    NumberOneStr, SumOfNumbersStr, FinalString: String;
Begin
    Error := NO_ERRORS;
    ...
    Try
        ...
        NumberOne := StrToInt(NumberOneStr);

        Read(ReadedFile, SumOfNumbersStr);
    Except
        Error := FILE_DATA_NOT_CORRECT;
    End;
    ...
    Begin
        ...
        FinalString := 'Сумма по формуле равна числу: ' + SumOfNumbersStr;
