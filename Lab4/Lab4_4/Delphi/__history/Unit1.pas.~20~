...
Function Partition(Var DataArray: TArrayOI; Low, High: Integer): Integer;
Var
    Pivot, RandomIndex, I, J: Integer;
    IsDoNotStop: Boolean;
Begin
    Pivot := 0;
    RandomIndex := 0;
    ...
    RandomIndex := Random(High - Low) + Low;
    Pivot := DataArray[RandomIndex];

    While IsDoNotStop Do
    Begin
        While (DataArray[I] < Pivot) Do
            Inc(I);

        While (DataArray[J] > Pivot) Do
            Dec(J);
...
Procedure Quicksort(Var DataArray: TArrayOI; Low, High: Integer);
Var
    Point: Integer;
Begin
    Point := 0;
    ...
    Begin
        If Low < High Then
        Begin
            Point := Partition(DataArray, Low, High);
...
Begin
    ...
    For I := 0 To ArrHigh Do
    Begin
        MainForm.StringGridResult.Cells[I, 1] := IntToStr( IntArray[I] );
    End;
...
Begin
    ...
    ArrLength := StrToInt(MainForm.NumberOneEdit.Text);
    ArrHigh := ArrLength - 1;
    StringGridAArray := GetStringArrayFromGrid(MainForm.StringGridA);
    StringGridResultArray := Copy(StringGridAArray);
    SortedArray := GetIntArrayFromStringArray(StringGridResultArray);
    Quicksort(SortedArray, 0, High(SortedArray));

    For I := 0 To ArrHigh Do
    Begin
        StringGridResultArray[I] := IntToStr(SortedArray[I]);
    End;
...
Var
    Error: ERROR_CODES;
    ArrLength, ArrHigh, I: Integer;
    ArrLengthStr, AnswerNumberStr, FinalString: String;
...
Begin
    ...
    Begin
        ...
        Begin
            ...
            SetLength(GridResultArray, ArrLength);
        End;
    End;
    ...
    Begin
        ...
        Quicksort(GridResultArray, 0, High(GridResultArray));
